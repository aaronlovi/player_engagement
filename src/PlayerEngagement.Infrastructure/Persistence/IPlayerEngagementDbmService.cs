...
public interface IPlayerEngagementDbmService
{
    ...
    Task<Result<PolicyVersionDTO>> GetPolicyVersionAsync(string policyKey, int policyVersion, CancellationToken ct);
    Task<Result<List<PolicyVersionDTO>>> ListPublishedPoliciesAsync(DateTime utcNow, CancellationToken ct);
    Task<Result<List<PolicyStreakCurveEntryDTO>>> GetPolicyStreakCurveAsync(string policyKey, int policyVersion, CancellationToken ct);
    Task<Result<List<PolicySeasonalBoostDTO>>> GetPolicySeasonalBoostsAsync(string policyKey, int policyVersion, CancellationToken ct);
