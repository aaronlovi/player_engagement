...
- Idempotent claims (deferred to claim flow): once claim orchestration exists, multiple attempts in the same reward day should return the same streak/XP result; streak state mutates only once per reward day.
