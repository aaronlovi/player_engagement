...
| -- | ----------------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------- | ------------------------------------------------ | ---------------- | --------------------------------------------------------------- |
| 1  | Environment Setup       | Initialize .NET Solution                    | Create a new .NET 8 solution named `XpService` with Orleans dependencies and `InnoAndLogic.Persistence` referenced. | None                       | `.sln` file and initial project structure.       | 1                | Use `dotnet new sln` and `dotnet new console` as base.          |
| 2  | Environment Setup       | Add PostgreSQL Connection Config            | Add environment variables or appsettings for PostgreSQL connection (`Host`, `Port`, `DB`, `User`, `Password`).      | Task 1                     | Config section in `appsettings.Development.json` | 1                | Match naming convention from tech reqs (`POSTGRES_HOST`, etc.). |
