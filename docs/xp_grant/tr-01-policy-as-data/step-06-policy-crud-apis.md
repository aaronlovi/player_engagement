...
| 6.3 | [ ] | **Application Services** | TODO: Implement orchestrators/services on top of `IPlayerEngagementDbmService`/`PolicyDocumentPersistenceService` for create/publish/retire along with scheduling logic. Read endpoints now call `PolicyDocumentPersistenceService` directly, but write paths remain unimplemented. | Unit tests (service layer) + integration tests using in-memory DBM. |
