...
| 3 | [ ] | **DBM service contract & impls** | Update `IPlayerEngagementDbmService` to expose `Task<Result<TDto>>` / `Task<Result<List<TDto>>>` for each query. Implement the methods in `PlayerEngagementDbmService` (Postgres) and `PlayerEngagementDbmInMemoryService`, ensuring the in-memory backing store can serve the same DTO shapes. |
| 4 | [ ] | **Mappers** | Add mapper classes in `src/PlayerEngagement.Infrastructure/Policies/Mappers/` that translate DTOs into domain types from `src/PlayerEngagement.Domain/Policies/` (e.g., `PolicyVersionMapper`, `PolicyDocumentMapper`, `PolicySegmentOverrideMapper`). Keep namespaces aligned with folders per `AGENTS.md`. |
